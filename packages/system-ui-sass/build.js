  ...
  sass.render({
    ...
    if (!err) {
     fs.writeFile(PATHS[`dist`], result.css, function(fsErr) {
      if (!fsErr) console.log(`${chalk.green('SUCCESS!')} ${OUTPUT_FILENAME} file generated!`);
      else console.log(`${chalk.red('ERROR!')} During ${OUTPUT_FILENAME} file generation: ${err}`);
     });
    } else {
     console.log(`${chalk.red('ERROR!')} During ${OUTPUT_FILENAME} file generation: ${err}`);
    }
  });
